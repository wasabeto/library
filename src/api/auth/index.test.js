...
test('POST /auth 400 (master) - invalid password', async () => {
  const { status, body } = await request(app()).post(apiRoot).query({ access_token: masterKey }).auth('a@a.com', '123');
  expect(status).toBe(400);
  expect(typeof body).toBe('object');
...
test('POST /auth 401 (master) - user does not exist', async () => {
  const { status } = await request(app()).post(apiRoot).query({ access_token: masterKey }).auth('b@b.com', '123456');
  expect(status).toBe(401);
});

test('POST /auth 401 (master) - wrong password', async () => {
  const { status } = await request(app()).post(apiRoot).query({ access_token: masterKey }).auth('a@a.com', '654321');
  expect(status).toBe(401);
});
