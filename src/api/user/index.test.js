...
test('POST /users 400 (master) - missing email', async () => {
  const { status, body } = await request(app()).post(apiRoot).send({ access_token: masterKey, password: '123456' });
  expect(status).toBe(400);
  expect(typeof body).toBe('object');
  expect(body.param).toBe('email');
});
...
test('POST /users 400 (master) - missing password', async () => {
  const { status, body } = await request(app()).post(apiRoot).send({ access_token: masterKey, email: 'd@d.com' });
  expect(status).toBe(400);
  expect(typeof body).toBe('object');
  expect(body.param).toBe('password');
});
...
test('POST /users 401', async () => {
  const { status } = await request(app()).post(apiRoot).send({ email: 'd@d.com', password: '123456' });
  expect(status).toBe(401);
});

test('DELETE /users/:id 204 (admin)', async () => {
  const { status } = await request(app()).delete(`${apiRoot}/${user1.id}`).send({ access_token: adminSession });
  expect(status).toBe(204);
});

test('DELETE /users/:id 401 (user)', async () => {
  const { status } = await request(app()).delete(`${apiRoot}/${user1.id}`).send({ access_token: session1 });
  expect(status).toBe(401);
});
